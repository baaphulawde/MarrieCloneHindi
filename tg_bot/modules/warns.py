...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    if is_user_admin(chat, user.id):
        message.reply_text("Damn admins, inko warn karne ke Liye jadu sikhna hoga @king_un_007 jadu sikhao bhai!")
        return ""
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "{} warnings, {} Laat marke nikala gya!".format(limit, mention_html(user.id, user.first_name))

        else:  # ban
            chat.kick_member(user.id)
            reply = "{} warnings, {} Pehli fursat me BAN kardiya!".format(limit, mention_html(user.id, user.first_name))
        ...
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Remove Warning ", callback_data="rm_warn({})".format(user.id))]])

        reply = "{} ke pass {}/{} warnings h ... sudharja beta!".format(mention_html(user.id, user.first_name), num_warns,
                                                             limit)
        if reason:
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if res:
            ...
            update.effective_message.edit_text(
                "All warnings of this user ya to 0 kardi ya fir pehle se 0 hi thi.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warns(bot: Bot, update: Update, args: List[str]):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = "is user ke pas {}/{} warnings h, for the following reasons:".format(num_warns, limit)
            ...
            update.effective_message.reply_text(
                "Is wale ke pass {}/{} warnings h, but no reasons for any of them.".format(num_warns, limit))
    else:
        update.effective_message.reply_text("Is user ke pas 0 warnings pehle se!")
...
def remove_warn_filter(bot: Bot, update: Update):
    ...
    for filt in chat_filters:
        if filt == to_remove:
            ...
            msg.reply_text("Yep, Warning nhi di jaegi is reason ke liye.")
            raise DispatcherHandlerStop
