...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text("Kisi ko mention ya tag karke fir try karo.")
        return ""
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text("Kaha milega ye?")
            return ""
        else:
            raise

    if is_user_ban_protected(chat, user_id, member):
        message.reply_text("admin h group me ban nhi ho sakta...")
        return ""

    if user_id == bot.id:
        message.reply_text("me pagal nhi hu jo khudko ban karuga?")
        return ""
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text("Well damn, kuch to gadbad h daya.")
...
def temp_ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Me gali nhi deta par mera bhai deta h @king_un_007 usko bol muje ban karne ko ?")
        return ""

    if not reason:
        message.reply_text("kitne time ke liye?!")
        return ""
    ...
    try:
        ...
        bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
        message.reply_text("Banned! ye user itne samay ke liye ban rahega {}.".format(time_val))
        return log

    except BadRequest as excp:
        if excp.message == "Reply message not found":
            # Do not reply
            message.reply_text("Banned! ye user itne samay ke liye ban rahega {}.".format(time_val), quote=False)
            return log
        else:
            LOGGER.warning(update)
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("Owner hi ek jadugar h jo admins ko kick kar sakta h..")
        return ""

    if user_id == bot.id:
        message.reply_text("Owner kis pagal ko admin diya h ye to muje hi kick kar rha h BSB?")
        return ""
...
def kickme(bot: Bot, update: Update):
    ...
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text("Apko kick karne ki boht icha thi...par ap admin nikle.")
        return

    res = update.effective_chat.unban_member(user_id)  # unban on current user = kick
    if res:
        update.effective_message.reply_text("Pehli fursat me (Apni marji se nikale gye ho).")
    else:
        update.effective_message.reply_text("Huh? kuch to gadbad h Daya :/")
...
def unban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Jadu wala aya group se Pagal Nikal...")
        return ""

    if is_user_in_chat(chat, user_id):
        message.reply_text("Aila Jadu!! ye user pehle se hi group me h")
        return ""

    chat.unban_member(user_id)
    message.reply_text("Usko link dedo join kar sakta h ab wo!")
...
def rban(bot: Bot, update: Update, args: List[str]):
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text("ye user admin h group me ban nhi ho sakta......")
        return

    if user_id == bot.id:
        message.reply_text("Arey bewde itna kyu pitey ho ki mujme aur jisse ban karna h usme farak bhul jao")
        return
...
__help__ = """
 - /kickme: Jisne isse use kiya usko group se nikala jaega (Ban nhi, kick)
