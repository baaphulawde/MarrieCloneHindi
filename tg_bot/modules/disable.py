...
if is_module_loaded(FILENAME):
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text("Command disable kiya gaya `{}`".format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Command disable nhi kiya ja saka (Kuch to gadbad h Daya)")

        else:
            update.effective_message.reply_text("Kya disable karna h?")
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("Is command ko Enable kiya gaya `{}`".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Is command ko disable nhi kiya gaya tha @king_un_007 se contact kare for more info ")

        else:
            update.effective_message.reply_text("Kya enable karna h?")
    ...
    def list_cmds(bot: Bot, update: Update):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("Koi command disable nhi kiya ja sakta.")
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "Koi command disable nhi h!"

        result = ""
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return "Ye diye gye commands disabled h :\n{}".format(result)
    ...
    def __stats__():
        return "{} commands disabled h, across {} chats.".format(sql.num_disabled(), sql.num_chats())
    ...
    __help__ = """
 - /cmds: Disabled commands ki list

*Admin only:*
 - /enable <cmd name>: enable that command
 - /disable <cmd name>: disable that command
 - /listcmds: Sabhi commands ki list
    """
