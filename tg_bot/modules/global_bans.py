...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in SUDO_USERS:
        message.reply_text("Bhabhi ko ban karega? @king_un_007 se baat kar tu ")
        return

    if int(user_id) in SUPPORT_USERS:
        message.reply_text("Bhabhi h panga mat le @king_un_007 ajaega!!!")
        return

    if user_id == bot.id:
        message.reply_text("-_- Isse bada pagal kon ho sakta h.")
        return
    ...
    if sql.is_user_gbanned(user_id):
        if not reason:
            message.reply_text("user already globbalyy ban ho chuka h...")
            return
    ...
    message.reply_text("⚡️ *Gets a punch* ⚡️")
